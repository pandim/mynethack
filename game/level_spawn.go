...
func (l *Level) spawnMonsters(count int) {
	if l == nil || count <= 0 || l.Width < 3 || l.Height < 3 { return }
	monsterTypes := []struct {
		name   string; hp, attack, gold, xp int; symbol rune; color tcell.Color
	}{
	...
	depth := l.Depth
	for i := 0; i < count; i++ {
		var x, y int; attempts := 0
		for {
			x = 1 + rand.IntN(l.Width-2); y = 1 + rand.IntN(l.Height-2)
			if l.Tiles[y][x].Type == TileFloor && !l.hasMonsterAt(x, y) && !l.hasItemAt(x, y) && !l.isStairsAt(x, y) { break }
			attempts++
			if attempts > MaxSpawnAttempts { return }
		}
		mt := monsterTypes[rand.IntN(len(monsterTypes))]
		hp := mt.hp * (1 + depth/2); attack := mt.attack * (1 + depth/3)
		gold := mt.gold * depth; xp := mt.xp + depth*2
...
func (l *Level) spawnItems(count int) {
	if l == nil || count <= 0 || l.Width < 3 || l.Height < 3 { return }
	itemTypes := []struct {
		name string; itype ItemType; value int; symbol rune; color tcell.Color
	}{
	...
	for i := 0; i < count; i++ {
		...
		for {
			x = 1 + rand.IntN(l.Width-2); y = 1 + rand.IntN(l.Height-2)
			if l.Tiles[y][x].Type == TileFloor && !l.hasItemAt(x, y) && !l.hasMonsterAt(x, y) && !l.isStairsAt(x, y) { break }
			attempts++
			if attempts > MaxSpawnAttempts { return }
		}
...
func (l *Level) spawnChests() {
	...
	for {
		x = 1 + rand.IntN(l.Width-2); y = 1 + rand.IntN(l.Height-2)
		if l.Tiles[y][x].Type == TileFloor && !l.hasMonsterAt(x, y) && !l.hasItemAt(x, y) && !l.isStairsAt(x, y) && !l.hasChestAt(x, y) { break }
		attempts++
		if attempts > MaxSpawnAttempts { return }
	}
...
func newChestContents() string {
	roll := rand.IntN(100)
	if roll < 50 { return "potion" } else if roll < 80 { return "food" }
	return "monster"
}

func newMerchantItems(depth, visitCount int) []*Item {
	basePrices := []struct {
		name string; itype ItemType; value, price int; symbol rune; color tcell.Color
	}{
