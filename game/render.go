...
func (g *Game) renderStartMenu() {
	...
	if hasSave {
		g.drawCentered(10, "Найдено сохранение игры!", highlightStyle)
	} else {
		g.drawCentered(10, "Добро пожаловать в подземелье!", style)
	}
...
//
// ⚠️ Экран разбит на 3 страницы, так как вся информация не помещается
...
//   - Страница 3 (helpPageMechanics): механики — свитки, реликвии, боссы
//
...
func (g *Game) renderHelpScreen() {
	...
	if g.helpPage == helpPageControls {
		...
		// СТРАНИЦА 3: МЕХАНИКИ
		// =====================================================================
		g.drawCentered(2, "=== МЕХАНИКИ ИГРЫ ===", titleStyle)
		...
		mechanics := []struct {
			...
			{"Золотые сундуки", "Требуют ключ (падает с врагов). Ценный лут"},
			{"Алтари (_)", "Благословляют экипировку (+2 ATK/DEF) за золото"},
			{"Боссы (G,N,D,B)", "Охраняют лестницу. Имеют особые способности"},
			{"Победа", "Убить Короля Бездны (25 ур.), забрать Амулет"},
			{"", "Вернуться на 1-й уровень с Амулетом Бездны!"},
		}
	...
	// =========================================================================
	// ИНДИКАТОР СТРАНИЦЫ И ПОДСКАЗКИ (общие для всех страниц)
	// =========================================================================
