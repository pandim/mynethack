...
func (g *Game) respawnLevel(l *Level) {
	...
	g.logAndSync("RESPAWN: Возрождение уровня %d", l.Depth)
	...
	// 4. Оживляем торговцев (новый товар)
	...
	for _, merchant := range l.Merchants {
		if merchant != nil {
			merchant.Items = newMerchantItems(l.Depth) // новый товар
		}
	}

	g.addMessage("Уровень изменился... Монстры вернулись!")
}